...
import numpy as np
import pytest
from src.utils import export_yolo_training_data

TEST_BUILD_KEY = "test_build_01"
DUMMY_HDF5_PATH = "/tmp/dummy_test_data.hdf5"
PROJECT_ROOT_FOR_TESTS = Path(export_yolo_training_data.PROJECT_ROOT)
...
def mock_config_paths(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(export_yolo_training_data, "DATASET_PATHS", mock_dataset_paths)
    monkeypatch.setattr(
        export_yolo_training_data.config, "DATASET_PATHS", mock_dataset_paths
    )
...
def mock_hdf5_file() -> mock.MagicMock:
    ...
    mock_file.__contains__.side_effect = lambda key: key in [
        export_yolo_training_data.CAMERA_PATH,
        export_yolo_training_data.SPATTER_MASK_HDF5_PATH,
        export_yolo_training_data.STREAK_MASK_HDF5_PATH,
        "slices/segmentation_results",  # for _check_mask_paths
    ]
    mock_file[export_yolo_training_data.CAMERA_PATH] = mock_camera_dataset
    ...
    mock_spatter_mask_dataset.shape = (100, 100)  # Match image shape
    mock_file[export_yolo_training_data.SPATTER_MASK_HDF5_PATH] = (
        mock_spatter_mask_dataset
    )
    ...
    mock_streak_mask_dataset.shape = (100, 100)  # Match image shape
    mock_file[export_yolo_training_data.STREAK_MASK_HDF5_PATH] = (
        mock_streak_mask_dataset
    )

    # Mock for listing available segmentation classes
    mock_segmentation_results = mock.MagicMock(spec=h5py.Group)
    mock_segmentation_results.keys.return_value = [
        str(export_yolo_training_data.CLASS_ID_SPATTER),
        str(export_yolo_training_data.CLASS_ID_STREAK),
    ]
...
def test_list_builds(mock_config_paths: None, capsys: pytest.CaptureFixture) -> None:
    ...
    with (
        ...
        export_yolo_training_data.main()
...
def test_main_no_build_key_error(
    ...
    with (
        ...
        # The script's main guard calls parser.error which exits
        export_yolo_training_data.main()
    assert e.value.code != 0  # argparse.error exits with 2
    captured = capsys.readouterr()
...
def test_export_data_for_yolo_success(
    ...
    mock_os_path_exists.return_value = True  # HDF5 file exists
    mock_h5py_file_constructor.return_value = mock_hdf5_file

    # Set the global paths within the script module
    monkeypatch.setattr(export_yolo_training_data, "DATA_PATH", DUMMY_HDF5_PATH)
    img_train_dir = (
    ...
    monkeypatch.setattr(export_yolo_training_data, "IMG_TRAIN_DIR", str(img_train_dir))
    monkeypatch.setattr(export_yolo_training_data, "LBL_TRAIN_DIR", str(lbl_train_dir))

    export_yolo_training_data.export_data_for_yolo()
    ...
    num_layers = mock_hdf5_file[export_yolo_training_data.CAMERA_PATH].shape[0]
    assert mock_imwrite.call_count == num_layers * 2  # 1 image + 1 label per layer

    # Check one image and one label call
    expected_img_path = str(img_train_dir / "00000.png")
    expected_lbl_path = str(lbl_train_dir / "00000.png")

    # Check that imwrite was called with the expected image path
    # and that the first argument to imwrite (the path) matches.
    image_call_found = False
    for call_args in mock_imwrite.call_args_list:
        if call_args[0][0] == expected_img_path:
            image_call_found = True
            assert isinstance(call_args[0][1], np.ndarray)  # image data
            break
    assert image_call_found, f"imwrite not called for image {expected_img_path}"

    label_call_found = False
    for call_args in mock_imwrite.call_args_list:
        if call_args[0][0] == expected_lbl_path:
            label_call_found = True
            assert isinstance(call_args[0][1], np.ndarray)  # label data
            # Verify label content based on mocked masks (e.g., check unique values)
            # For simplicity, we just check it's an ndarray here.
            break
    assert label_call_found, f"imwrite not called for label {expected_lbl_path}"
...
def test_export_data_hdf5_not_found(
    ...
    monkeypatch.setattr(export_yolo_training_data, "DATA_PATH", DUMMY_HDF5_PATH)
    monkeypatch.setattr(
        export_yolo_training_data, "IMG_TRAIN_DIR", "/tmp/img"
    )  # Need to set these
    monkeypatch.setattr(export_yolo_training_data, "LBL_TRAIN_DIR", "/tmp/lbl")
...
def test_export_data_hdf5_missing_camera_path(
    ...
    mock_file = mock.MagicMock(spec=h5py.File)
    mock_file.__enter__.return_value = mock_file
    mock_file.__contains__.return_value = False  # CAMERA_PATH does not exist
    mock_file.keys.return_value = ["some_other_key"]
    mock_h5py_file_constructor.return_value = mock_file

    monkeypatch.setattr(export_yolo_training_data, "DATA_PATH", DUMMY_HDF5_PATH)
    monkeypatch.setattr(export_yolo_training_data, "IMG_TRAIN_DIR", "/tmp/img")
    monkeypatch.setattr(export_yolo_training_data, "LBL_TRAIN_DIR", "/tmp/lbl")

    export_yolo_training_data.export_data_for_yolo()
    assert (
        f"CAMERA_PATH '{export_yolo_training_data.CAMERA_PATH}' not found in HDF5 file."
        in caplog.text
    )
...
def test_export_data_hdf5_missing_mask_paths(
    ...
    mock_os_path_exists: mock.MagicMock,
    mock_config_paths: None,
    mock_hdf5_file: mock.MagicMock,  # Use parts of the more complete mock
    caplog: pytest.LogCaptureFixture,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    # Modify the mock_hdf5_file to simulate missing mask paths
    mock_hdf5_file.__contains__.side_effect = (
        lambda key: key == export_yolo_training_data.CAMERA_PATH
    )
    # Ensure 'slices/segmentation_results' does not exist for the warning path
...
def test_script_execution_list_builds(
    ...
    result = subprocess.run(
        ...
        capture_output=True,
        text=True,
        check=False,  # Allow non-zero exit for SystemExit(0)
        cwd=PROJECT_ROOT_FOR_TESTS,  # Run from project root
    )
